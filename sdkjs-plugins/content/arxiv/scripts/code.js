...
(function () {
    ...
    window.Asc.plugin.init = function () {
        ...
        sdk = window.Asc.plugin.arxiv.api({});
        ...
        function searchFor(text, page = 0) {
    ...
    function displaySearchItems(append, res, err, hideLoader, isGroup, showNotFound) {
        ...
        if (res.items.length === 0 && showNotFound) {
        ...
        res.items.forEach(function (item) {
            var docElement = buildDocElement(item);
            holder.appendChild(docElement);
        });

        // 添加分页控件
        if (!append && res.totalResults > 10) {
            var paginationElement = buildPaginationElement(res.currentPage, Math.ceil(res.totalResults / 10));
            holder.appendChild(paginationElement);
        }

        docsScroller.onscroll();
    }

    function buildPaginationElement(currentPage, totalPages) {
        ...
        prevBtn.onclick = function(e) {
            e.stopPropagation();
            if (currentPage > 0) {
                loadLibrary(sdk.search(lastSearch.text, currentPage - 1), false, true, true, false, true);
            }
        };
        ...
        for (var i = 0; i < buttons.length; i++) {
            buttons[i].style.border = '1px solid #ddd';
	...
	function formatInsertLink() {
        ...
        try {
			...
			window.Asc.plugin.executeMethod("AddAddinField", [field], function() {
				showLoader(false);
				// TODO есть роблема, что в плагине мы индексы обновили, а вот в документе нет (по идее надо обновить и индексы в документе перед вставкой)
    ...
    function convertToCSL(item) {
        var cslData = {
            id: item.id,
            title: item.title,
            author: item.author.map(a => ({ family: a.name.split(' ').slice(-1)[0], given: a.name.split(' ').slice(0, -1).join(' ') })),
